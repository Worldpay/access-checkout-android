...
internal class CardConfigurationParser : Deserializer<CardConfiguration>() {
    ...
    private fun parseBrandsConfig(root: JSONObject): List<CardBrand>? {
        ...
        return brandsArray?.let {
            ...
            for (i in 0 until it.length()) {
                ...
                val cvv = fetchOptionalObject(brandRoot,
                    BRANDED_CVV_FIELD
                )
                val cvvConfig = cvv?.let { cvvRule -> parseCardValidationRule(cvvRule) }
                val pans = fetchArray(brandRoot,
                    PANS_FIELD
                )
    ...
    private fun parseDefaultConfig(jsonObject: JSONObject): CardDefaults? {
        val defaults = fetchOptionalObject(jsonObject,
            DEFAULTS_FIELD
        )
        return defaults?.let {
            val pan = fetchOptionalObject(it,
                PAN_FIELD
            )
            val cvv = fetchOptionalObject(it,
                CVV_FIELD
            )
            val month = fetchOptionalObject(it,
                MONTH_FIELD
            )
            val year = fetchOptionalObject(it,
                YEAR_FIELD
            )
    ...
    private fun parseCardValidationRule(jsonObject: JSONObject): CardValidationRule {
        val matcher = toOptionalStringProperty(jsonObject,
            MATCHER_FIELD
        )
        val minLength = toOptionalIntProperty(jsonObject,
            MIN_LENGTH_FIELD
        )
        val maxLength = toOptionalIntProperty(jsonObject,
            MAX_LENGTH_FIELD
        )
        val validLength = toOptionalIntProperty(jsonObject,
            VALID_LENGTH_FIELD
        )
    ...
    private fun parseSubRules(jsonObject: JSONObject): List<CardValidationRule> {
        val subRules = fetchOptionalArray(jsonObject,
            SUB_RULES_FIELD
        )
