...
class AccessCheckoutCVVTest {
    ...
    @Test
    fun `should raise an onUpdate event with valid state twice and onUpdateLengthFilter event thrice - when cvv is partially valid and onUpdateCVV is called`() {
        givenCvvIsOnFocus()
        givenCvvIsPartiallyValid()

        createNewAccessCheckoutCVV().onUpdateCVV(cvvValue)

        verify(cardListener, times(2)).onUpdate(cvvView, true)
        verify(cardListener, times(3)).onUpdateLengthFilter(cvvView, cvvLengthFilter)
    }

    @Test
    fun `should raise an onUpdate event with valid state twice and onUpdateLengthFilter event twice - when cvv is complete and valid and onUpdateCVV is called`() {
        givenCvvIsCompletelyValid()
    ...
    @Test
    fun `should raise an onUpdate event with valid state twice and onUpdateLengthFilter event twice - when cvv is invalid and onUpdateCVV is called`() {
        givenCvvIsInvalid()
    ...
    @Test
    fun `should raise an onUpdate event with invalid state twice - when cvv is partially valid and onEndUpdateCVV is called`() {
        givenCvvIsOffFocus()
        givenCvvIsPartiallyValid()

        createNewAccessCheckoutCVV().onEndUpdateCVV(cvvValue)

        verify(cardListener, times(2)).onUpdate(cvvView, false)
    }

    @Test
    fun `should raise an onUpdate event with valid state twice - when cvv is complete and valid and onEndUpdateCVV is called`() {
        givenCvvIsOffFocus()
        givenCvvIsCompletelyValid()
    ...
    @Test
    fun `should raise an onUpdate event with invalid state twice - when cvv is invalid and onEndUpdateCVV is called`() {
        givenCvvIsOffFocus()
        givenCvvIsInvalid()
    ...
    fun givenMethodsAreCalled() {
        ...
        val accessCheckoutCVV = AccessCheckoutCVV(cvvView)
        accessCheckoutCVV.onUpdatePAN("4444444444444")
        accessCheckoutCVV.onEndUpdatePAN("4444444444444")

        accessCheckoutCVV.onUpdateDate(null, null)
        accessCheckoutCVV.onEndUpdateDate(null, null)
